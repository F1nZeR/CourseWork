...
namespace ODIS.AIM
{
    ...
    public class MAPStream : RandomEventStream
    {
        ...
        /// <summary>
        /// Сохраняем предыдущее состяние управляющего процесса и переходим в нем к следующему
        /// </summary>
        private void ControlProcessNext()
        {
            oldControlState = (int)ControlProcess.State;
            ControlProcess.Next(); 
        }
        ...
        public override double NextValue()
        {
            ...
            if (!IsStateChanging)
            {
                ...
                int newState = (int)ControlProcess.State;

                // решаем, надо ли производить событие в момент смены состояний (матрица D)
                IsStateChanging = (baseGenerator.NextValue() < D[oldControlState, newState]);
                if (IsStateChanging)
