...
namespace ODIS.WinApp
{
    public partial class frmQueueStatistic : Form
    {
        ...
        public frmQueueStatistic(QueueSimulationModel model, ServerBlock node)
        {
            InitializeComponent();
            this.Model = model;
            if (node.Text.Length > 0) Text += " " + node.Text;
            panelStatistic.SetGeneration(node.InSystemStatistic);
            panelStatistic.SetDistribution(Model.GetEstimateDistributionForQueue());
            btnBuffer.Enabled = node.Buffer.BasicStatistic.Count() > 0;
        }

        public static void ShowStatistic(QueueSimulationModel model, ServerBlock node)
        {
            frmQueueStatistic frm = new frmQueueStatistic(model, node);
            frm.Show();
        }
